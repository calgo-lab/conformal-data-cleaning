...
class PrecisionForestCleaner(BaseCleaner):
    ...
    def _fit_method(self, data: pd.DataFrame, **kwargs: dict[str, Any]) -> PrecisionForestCleaner:
        ...
        for column in self.target_columns_:
            ...
            if column in self._categorical_columns:
                ...
                self._classification_threshold[column] = {}
            ...
            if column in self._categorical_columns:
                ...
                for label_idx, label in enumerate(self.predictors_[column].named_steps["predictor"].classes_):
                    ...
                    threshold_for_minimal_precision = threshold[
                        (precision >= self.confidence_level_).nonzero()[0][0] - 1  # -1 because: len(threshold) == len(rec) -1
                    ]

                    self._classification_threshold[column][label] = threshold_for_minimal_precision
    ...
    def _remove_outliers_method(
        ...
        **kwargs: dict[str, Any],
        ...
        for column in self.target_columns_:
            ...
            # If there is any (high-precision) prediction that differs from the cell, it's probably an outlier
            if column in self._categorical_columns:
                probabilities = self.predictors_[column].predict_proba(data[[col for col in data.columns if col != column]])

                for label_idx, label in enumerate(self.predictors_[column].named_steps["predictor"].classes_):
                    above_precision_predictions = self._classification_threshold[column][label] <= probabilities[:, label_idx]
                    outliers[column] = above_precision_predictions & (data[column] != y_prediction)

                conformalized_predictions = []  # NOTE: this is just to avoid downstream issues, we do not have prediction sets here
